...
{ // isset, parametre olarak aldığı değişkenin tanımlı olup olmadığını kontrol eder.
	...
	if($email && $password)
	{
		...
		$query = mysqli_query($baglan, "SELECT *,CONCAT(musteri.adi,' ',musteri.soyadi) AS AdSoyad FROM girisbilgileri,musteri 
		WHERE girisbilgileri.girisBilgiID = musteri.girisBilgiID 
		AND email='$email' AND password='$password'");
		$musteriSayisi = mysqli_num_rows($query);
		
		if($musteriSayisi > 0)
		{
			// Giriş Yapan Personel Değil
				$query = mysqli_query($baglan, "SELECT *,CONCAT(musteri.adi,' ',musteri.soyadi) AS AdSoyad FROM girisbilgileri,musteri 
				WHERE girisbilgileri.girisBilgiID = musteri.girisBilgiID 
				AND email='$email' AND password='$password'");
				$row = mysqli_fetch_array($query);
						...
						<div class="card-body">
							...
							<form method="POST" class="my-login-validation" novalidate="" action="login.php">
								<div class="form-group">
									...
									<input type="name" class="form-control" name="tBox_emailGiris" value="" required autofocus>
